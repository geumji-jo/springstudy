...
public class FirstController {
	...
	// FirstServiceImpl을 SpringContainer에 Bean으로 등록시켜보자.(root-context.xml에
	// <bean>태그등록)
	// Spring Container에 등록된 firstService Bean을 아래 필드 firstService에 주입해보자. (필드에
	// @Autowired 추가)
	...
	public Person ajax1(HttpServletRequest request, HttpServletResponse response) { // "Jackson 라이브러리"가 반환값 Person 객체를
																					// 자동으로 JSON 데이터로 변환한다.
	...
	public Map<String, Object> ajax3(Person person) { // "Jackson 라이브러리"가 반환값 Person 객체를																			// 자동으로 JSON 데이터로 변환한다.
