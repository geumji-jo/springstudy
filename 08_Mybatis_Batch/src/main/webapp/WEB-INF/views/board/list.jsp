...
<script>
	...
	function fnChkOne() {
		...
		chkOne.on('click', function() {
			let chkCnt = 0;
			for(let i = 0; i < $('.chk_one').length; i++) {
	...
	<div>
		<form id="frm_remove_list" action="${contextPath}/board/removeList.do"method="post">
			...
			<table border="1">
				<thead>
					<tr>
						<td><label for="chk_all" id="lbl_chk_all">전체선택</label> <input
							type="checkbox" id="chk_all"></td>
						<td>제목</td>
						<td>작성자</td>
						<td>작성일자</td>
					</tr>
				...
				<tbody>
					<c:if test="${empty boardList}">
						<tr>
							<td colspan="4">첫 글의 주인공이 되어보세요</td>
						</tr>
					</c:if>
					<c:if test="${not empty boardList}">
						<c:forEach items="${boardList}" var="b">
							<tr>
								<td><input type="checkbox" class="chk_one" name="boardNoList"value="${b.boardNo}">
									<img src="${contextPath}/resources/">
								</td>
								<td><a
									href="${contextPath}/board/detail.do?boardNo=${b.boardNo}">${b.title}</a></td>
								<td>${b.writer}</td>
								<td>${b.createdAt}</td>
							</tr>
						</c:forEach>
