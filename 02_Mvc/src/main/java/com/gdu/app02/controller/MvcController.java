...
public class MvcController {
	...
	/*
		...
		모든 요청마다(contextPath를 가진 모든 요청)마다 동작하도록 filter를 사용한다.
	...
	public String detailMe(@RequestParam(value="name", required=false, defaultValue="홍길동" ) String name, 		// 요청 파라미터name이 없으면 "홍길동"을 사용한다.
			               @RequestParam(value="age", required=false, defaultValue="0" ) int age, // 요청 파라미터age이 없으면 "0"을 사용한다.
			               Model model) {
		model.addAttribute("name", name);
		model.addAttribute("age", age);
	...
	public String detailMe(String name, 		// 요청 파라미터name이 없으면 null이 저장된다.
			               int age,				// 요청 파라미터age이 없으면 null을 int로 변환하려고 하기 때문에 오류가 발생한다.
			               Model model) {
		model.addAttribute("name", name);
		model.addAttribute("age", age);
	...
	public String detailGdu(Person p) {		// name과 age를 필드로 가진 커맨드 객체 Person P
		...
		@GetMapping("/detail.gdu")
		public String detailGdu(@ModelAttribute(value="p") Person p) {		// Model에 저장할 속성명을 p로 해주세요.
