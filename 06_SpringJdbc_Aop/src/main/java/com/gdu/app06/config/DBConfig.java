...
public class DBConfig {
	...
	// JdbcTemplate Bean(Connection, PreparedStatement, ResultSet 을 이용해서 동작하는 스프링 클래스)
	@Bean
	...
	@Bean
	public TransactionManager transactionMagnager() {
	...
	public TransactionInterceptor transactionInterceptor() {
		...
		ruleBasedTransactionAttribute.setRollbackRules(Collections.singletonList(new RollbackRuleAttribute(Exception.class)));//언제 취소를 할것입니까?
	
		MatchAlwaysTransactionAttributeSource source = new MatchAlwaysTransactionAttributeSource();
		source.setTransactionAttribute(ruleBasedTransactionAttribute);
		
		return new TransactionInterceptor(transactionMagnager(), source);
	...
	/*
		AOP 관련 핵심 용어 3가지
		1. 조인 포인트(JoinPoint)	: AOP를 적용시킬 수 있는 메소드 전체		- 목록, 상세, 삽입, 수정, 삭제
		2. 포인트컷(PointCut)		: 조인 포인트 중에서 AOP를 동작시킬 메소드	- 삽입, 수정, 삭제
